#!/usr/bin/env python
# -*- coding: UTF-8 -*-
import xbmcplugin,xbmcgui,xbmcaddon,urllib,re,os,xbmc,sys

myaddon = xbmcaddon.Addon(id="plugin.video.thietkeweb30s.org.fshare")
...
def mess(message='',title='',timeShown=5000):
	if not message:xbmc.executebuiltin("Dialog.Close(all, true)")
		...
		s0='[COLOR green][B]Thietkeweb30s.org[/B][/COLOR]'+title
...
def loginfshare(headers={'User-Agent':'Mozilla/5.0 Chrome/39.0.2171.71 Firefox/33.0'}):
	...
	else:mess(u'Login không thành công. Tên đăng nhập và mật khẩu chưa chính xác!','Fshare.vn')
	return result
